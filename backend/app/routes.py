...
# ✅ CORS no blueprint apenas para /api/*
# Em produção, troque origins="https://vocationaltest.com.br"
CORS(main, resources={r"/api/*": {"origins": "*"}}, supports_credentials=False)
...
# ---------- Headers anti-cache (útil no iOS) ----------
...
# ---------- Endpoints ----------
@main.route('/api/chat-vocacional', methods=['POST', 'OPTIONS'])
def chat_vocacional():
    ...
    try:
        ...
        # INÍCIO DA CONVERSA LIVRE (kickoff)
        if kickoff:
            contexto = montar_contexto_do_chat(chat_id)
            msgs = [
                {"role": "system", "content":
        ...
        mensagens_completas = [
            {"role": "system", "content":
                ...
                "SEM cumprimentos do tipo 'Olá'. Sempre finalize com UMA pergunta curta."},
            {"role": "user", "content":
                f"Contexto (últimas interações de pergunta e resposta):\n{contexto_compacto}\n\n"
                f"Nova mensagem do usuário: {pergunta}\n\n"
                f"Considere o contexto e evite repetir perguntas já feitas."}
        ]
...
@main.route('/api/analise-perfil', methods=['POST', 'OPTIONS'])
def analise_perfil():
    data = request.get_json(silent=True) or {}
    chat_id = data.get('chat_id', gerar_chat_id())
    ...
    try:
        ...
        return jsonify({"analise": f"Erro no servidor: {str(e)}"}), 500
